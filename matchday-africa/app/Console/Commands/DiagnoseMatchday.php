...
class DiagnoseMatchday extends Command {
    ...
    public function handle():int{
        ...
        $apiFootballKey=(string)config('services.api_football.key');
        $checks=[['Football API key',$footballKey?'Configured':'MISSING',''],['Event API key',$apiFootballKey?'Configured':'OPTIONAL','API-Football goals, cards and statistics'],['OpenRouter key',$openRouterKey?'Configured':'MISSING',''],['X posting credentials',config('services.twitter_api.access_token')?'Configured':'CHECK DATABASE','Hourly engagement requires a persisted OAuth token or access token']];
        ...
        if($apiFootballKey){try{$r=Http::timeout(15)->withHeaders(['x-apisports-key'=>$apiFootballKey])->get(rtrim(config('services.api_football.url'),'/').'/status');$checks[]=['Event API HTTP',$r->status(),$r->successful()?'Reachable · remaining today '.($r->json('response.requests.current')!==null?max(0,(int)$r->json('response.requests.limit_day')-(int)$r->json('response.requests.current')):'unknown'):$this->safeError($r)];}catch(\Throwable $e){$checks[]=['Event API HTTP','ERROR',$e->getMessage()];}}
