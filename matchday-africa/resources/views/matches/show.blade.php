...
<div class="md-scoreboard"><div>@if($warTeams['home'])<img class="md-war-avatar" src="{{ $warTeams['home'] }}" alt="{{ $match->homeTeam?->name }} warrior">@endif<strong>{{ $match->homeTeam?->name }}</strong></div><b><span id="match-home-score">{{ $match->home_score ?? '–' }}</span> <i>:</i> <span id="match-away-score">{{ $match->away_score ?? '–' }}</span></b><div>@if($warTeams['away'])<img class="md-war-avatar" src="{{ $warTeams['away'] }}" alt="{{ $match->awayTeam?->name }} warrior">@endif<strong>{{ $match->awayTeam?->name }}</strong></div></div>
...
<div class="md-room-actions">@if($predictionSet)<a class="md-primary" href="{{ route('predictions.show',$predictionSet) }}">Make your call</a>@endif<a class="md-primary" href="{{ route('war.match',$match) }}#game">{{ $match->homeTeam?->display_name }} vs {{ $match->awayTeam?->display_name }} · Kick off</a><button class="md-secondary md-share" data-share-title="{{ $match->homeTeam?->name }} vs {{ $match->awayTeam?->name }}" data-share-text="Join the match room on Matchday Africa.">Share match</button></div></div></section>
