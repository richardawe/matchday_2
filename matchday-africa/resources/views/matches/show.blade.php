...
<div class="md-room-actions">@if($predictionSet)<a class="md-primary" href="{{ route('predictions.show',$predictionSet) }}">Make your call</a>@endif<a class="md-primary" href="{{ route('war.match',$match) }}">{{ $match->homeTeam?->display_name }} vs {{ $match->awayTeam?->display_name }} · Kick off</a><button class="md-secondary md-share" data-share-title="{{ $match->homeTeam?->name }} vs {{ $match->awayTeam?->name }}" data-share-text="Join the match room on Matchday Africa.">Share match</button></div></div></section>
